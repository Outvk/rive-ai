...
import { PaperPlaneIcon, UpdateIcon, PlayIcon } from '@radix-ui/react-icons'
...
export function TextToSpeechForm({ initialCredits = 0 }: { initialCredits?: number }) {
   ...
   const handlePreview = async (voice: Voice) => {
    ...
    className={`relative flex flex-col items-start px-3 py-2.5 rounded-lg border text-left transition-all cursor-pointer ${
        selectedVoice?.id === voice.id
            ? 'bg-amber-500/15 border-amber-500/50 text-amber-300'
            : 'bg-zinc-800/40 border-white/10 text-zinc-300 hover:bg-zinc-800/70 hover:border-white/20'
    }`}
>
    <span className="text-sm font-medium truncate w-full pr-6">{voice.name}</span>
    {voice.preview_url && (
        <button
            type="button"
            onClick={(e) => {
            ...
            className="absolute top-2 right-2 p-1 rounded-md bg-zinc-700/50 hover:bg-amber-500/30 text-zinc-400 hover:text-amber-400 transition-colors"
            title="Preview voice"
        >
            <PlayIcon className="w-3 h-3" />
        </button>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 pt-6">
                <textarea
                    ...
                    className="w-full bg-zinc-800/40 backdrop-blur-xl rounded-lg p-4 text-zinc-100 placeholder:text-zinc-500 resize-none min-h-[140px] border border-white/10 focus:outline-none focus:ring-2 focus:ring-amber-500/50"
                    rows={5}
                />
                <div className="flex items-center justify-between">
                    <button
